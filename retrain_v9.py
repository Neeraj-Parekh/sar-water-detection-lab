...
"""
================================================================================
RETRAIN V9 - Clean Data ONLY with Optional MNDWI
================================================================================

This version trains ONLY on chips_expanded_npy/ (clean, validated data).
This allows fair comparison:
  - v7: SAR-only on clean data -> IoU 0.794
  - v9: SAR+MNDWI on same clean data -> ???

Goal: Determine if MNDWI improves accuracy on clean data.
...
CONFIG = {
    "version": "9.0",
    "random_seed": 42,
    # ONLY clean data - no corrupted chips/
    "chip_dirs": [
        Path("/home/mit-aoe/sar_water_detection/chips_expanded_npy"),  # Clean validated
    ],
    "model_dir": Path("/home/mit-aoe/sar_water_detection/models"),
    "results_dir": Path("/home/mit-aoe/sar_water_detection/results"),
    "test_size": 0.2,
    "sample_rate": 0.15,  # Same as v7 for fair comparison
    "lgb_params": {
        "objective": "binary",
        "metric": "binary_logloss",
        "boosting_type": "gbdt",
        "num_leaves": 63,  # Same as v7
        "max_depth": 8,  # Same as v7
        "learning_rate": 0.05,
...
def extract_features(
    ...
    """
    Extract comprehensive feature set (same as v7 + MNDWI).
    """
    vv = data["vv"]
    vh = data["vh"]
    dem = data["dem"]
...
def load_all_chips(chip_dirs: List[Path]) -> List[Dict]:
    """Load all chips from directories."""
    all_chips = []
...
def main():
    ...
    logger.info("=" * 70)
    logger.info("RETRAIN V9 - Clean Data ONLY with MNDWI")
    logger.info("=" * 70)
    logger.info("This trains on chips_expanded_npy/ ONLY (no corrupted chips/)")
    logger.info("Goal: Compare with v7 (SAR-only) to see if MNDWI helps")

    # Create output directories
    CONFIG["model_dir"].mkdir(parents=True, exist_ok=True)
    CONFIG["results_dir"].mkdir(parents=True, exist_ok=True)

    # Load chips (ONLY from chips_expanded_npy)
    logger.info("Loading chips from clean directories only...")
    ...
    # Save model
    model_name = "lightgbm_v9_clean" + ("_mndwi" if use_mndwi else "")
    ...
    logger.info("\n" + "=" * 70)
    logger.info("RESULTS SUMMARY - V9 (Clean Data + MNDWI)")
    logger.info("=" * 70)
    ...
    logger.info(f"\nComparison with V7 (SAR-only, clean):")
    logger.info(f"  V7 IoU: 0.7936")
    logger.info(f"  V9 IoU: {overall_iou:.4f}")
    logger.info(f"  Improvement: {(overall_iou - 0.7936) * 100:.2f}%")
    ...
    results = {
        ...
        "data_source": "chips_expanded_npy ONLY (clean)",
        "comparison_v7_iou": 0.7936,
    ...
    results_path = (
        CONFIG["results_dir"]
        / f"retrain_v9_results{'_mndwi' if use_mndwi else ''}.json"
    )
    ...
    logger.info("=" * 70)
    logger.info("V9 TRAINING COMPLETE")
    logger.info("=" * 70)
